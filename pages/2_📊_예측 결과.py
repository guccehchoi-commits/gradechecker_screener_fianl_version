...
with col_f1:
    view_mode = st.radio('표시 범위', ['전체', '고위험만', '검토 대상만', '이상 없음만'], horizontal=True)
with col_f2:
...
col_rename = {'game_name': '게임명', 'grade': '등급', 'company': '플랫폼', 'genre': '장르'}
...
df_display['재분류_확률'] = df_display['재분류_확률'].apply(lambda v: f'{v:.4f}')
st.dataframe(df_display, use_container_width=True, height=420)
if 'game_name' in df_view.columns:
    ft_rows = df_view[df_view['game_name'].apply(is_fortnite)]
    if len(ft_rows) > 0:
        st.warning(f'⚠️  **포트나이트 관련 게임 {len(ft_rows)}건** — 학습 데이터에 이상치 이력이 없어 예측 신뢰도가 낮습니다.', icon='⚠️')
st.divider()
...
with col_dl1:
    ...
    st.download_button(label='⬇  CSV 다운로드', data=csv, file_name=f'이상탐지_결과_{stem}.csv', mime='text/csv', type='primary', use_container_width=True)
with col_dl2:
    import io
    buf = io.BytesIO()
    df_dl.to_excel(buf, index=True, engine='openpyxl')
    st.download_button(label='⬇  Excel 다운로드', data=buf.getvalue(), file_name=f'이상탐지_결과_{stem}.xlsx', mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', type='primary', use_container_width=True)
