...
### Fixed
- **장르 기반 보정이 실제로는 작동하지 않던 문제 수정.** `BETTING_GENRES`가
  `'보드게임(베팅성)'` 정확일치라 GRAC 자체등급분류 표기 `'보드(베팅성)'`과 맞지 않아
  크롤링 데이터에서 매칭 0건이었음(기관등급분류 데이터에서만 동작).
  `'카지노'` 장르가 목록에 없었고 `'카지노,기타'` 같은 복합 표기도 놓쳤음.

### Changed
- `apply_genre_boost()` 재작성 — 부분일치 + 청소년이용불가 여부 조건 추가
  - 장르에 `카지노` 포함 + 청불 아님 → `max(p + 0.30, 0.85)`
  - `보드(베팅성)` 계열 + 청불 아님 → `p + 0.10` (최소값 없음)
- `보드(베팅성)`을 약하게 둔 근거: 2026년 7~8월 신규 등록분 실측 시 해당 장르
  359건 중 97%가 레이싱 게임 등 사업자의 장르 오신고였음.

### Impact (자체 테스트셋 53,437건 기준)
- 장르가 카지노인데 청불이 아닌 건의 탐지율: **25.6% → 100%**
- 미학습 정상 데이터 오탐률: 8.09% → 8.30% (+0.21%p)
- 이상 데이터 탐지율: 80.9% (변동 없음)
- `55 club`, `dpboss`, `79king`, `Aviator Tips` 등 게임명만으로는 판별 불가하던 건 신규 적발

### Files Changed
- `utils/preprocess.py`
- `pages/3_🔍_예측_결과_분석.py` (보정 내역 안내 문구)
